...
const smartsheet = smarclient.createClient({
    ...
    accessToken: 'Bearer 2a24ncrb8yyasgbjb235tcyk6o'
});
    ...
    smartsheet.tokens.getAccessToken(options, processToken)
        .then((token) => {
            return res
                .status(200)
                // .json(token);
                .redirect('/callback-success')
        });
    // res.render('index.html');
});

app.get('/callback-success', (req, res) => {
    console.log("here we got the token : ");
    let _sessionData = require('./token_priv.json');
...
function processToken(error, token) {
    ...
    // localStorage.setItem('_token', JSON.stringify(returned_token));
